...
上面这些模型都在 **设置 → 模型配置** 里统一管理：列表按用途分组（角色识别 / 头部检测 / 训练集打标 / 参考图匹配），每一行显示名称、大小、来源仓库和状态（随包提供 / 已就绪 / 不完整 / 未下载 / 使用中），可以单独下载、删除，也可以把某个识别模型设为「当前识别模型」。面板上方同时显示模型目录（`models\`）和 Hugging Face 缓存目录，点「打开目录」可直接查看文件。
