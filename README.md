...
| 模型配置 | **所有内置 / 可下载 / 已缓存的模型**：角色识别、头部检测、WD14 打标、CCIP 参考匹配；可下载、删除、把某个识别模型设为「当前识别模型」，并显示模型目录与缓存目录 |
...
| 角色识别（可选） | `camie-initial`（deepghs Camie tagger） | **70,527 标签**，含大量冷门角色；824MB，装好后在设置 → 模型配置里切换 |
| 参考图相似度 | CCIP（`deepghs/ccip_onnx`） | 与识别模型解耦，实测判别力最好；约 143MB，可在设置 → 模型配置提前下载 |
| 打标（训练集） | WD14 `SwinV2_v3` | 约 446MB，可在设置 → 模型配置提前下载；不下载则首次导出时才拉取 |
...
上面这些模型都在 **设置 → 模型配置** 里统一管理：列表按用途分组（角色识别 / 头部检测 / 训练集打标 / 参考图匹配），每一行显示名称、大小、来源仓库和状态（随包提供 / 已就绪 / 不完整 / 未下载 / 使用中），可以单独下载、删除，也可以把某个识别模型设为「当前识别模型」。面板上方同时显示模型目录（`models\`）和缓存目录（`data\hf-cache\`），点「打开目录」可直接查看文件。

打标模型、参考匹配模型和训练时的下载都固定写在软件目录里的 `data\hf-cache\`，不落 `%USERPROFILE%`、`AppData` 或系统临时目录，所以便携包拷到别的机器上模型仍然在。如果本机别处（`HF_HOME`、`.cache\huggingface` 等）已经存有这些模型，模型配置里会出现**「复制到软件目录」**按钮，一键搬进软件缓存，省掉几个 GB 的重复下载。
